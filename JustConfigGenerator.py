...
def main():
	...
	LoopIterations=[3,4] #,5,6];
	MbyteSize=33 # 2^28=32Mbyte= 2^20[1M] * 2^5 [32] * 2^3[byte]
	MaxSize=2**MbyteSize
	...
	MasterSWStatsSuffix='_Size2power'+str(MbyteSize)+'_dim'+str(Max['Dims'])+'to'+str(Min['Dims']+1)+'_Stride'+str(Min['Stride'])+'to'+str(Max['Stride'])+'_Streams'+str(Min['NumStream'])+'to'+str(Max['NumStream'])+'_Iterations'+str(LoopIterations[len(LoopIterations)-1])+'to'+str(LoopIterations[0])
				...
				for NumStreams in range(Min['NumStream'],Max['NumStream']+1):
					...
					for CurrStreamCombi in ResultString:
						...
						for CurrAlloc in Alloc:
							...
							SRCID='Iters'+str(NumLoopIterations)+'_'+str(NumVars)+"vars_"+str(CurrAlloc)+'_'+str(NumDims)+"dims_"+str(SizeName)+'_streams_'+str(NumStreams)+'_maxstride_'+str(MaxStride)
